...
typedef enum _CXTransactionType {
  Income = 0,
  Expense
} CXTransactionType;

typedef enum _CXFontSize {
  Smaller = 0,
  Bigger
} CXFontSize;

typedef enum _CXCurrencyPosition {
  Before = 0,
  After
} CXCurrencyPosition;

typedef enum _CXSeparator {
  Comma = 0,
  Period
} CXSeparator;

typedef enum _CXDateFormat {
  DDMMYY = 0,          // 24-12-02
